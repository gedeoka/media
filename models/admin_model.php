...
class Admin_Model extends Model {
	...
	public function DelContact($id){
        $sts=$this->db->prepare('Delete from customercontact where id=:id');
        $sts->execute(array(
            ':id'=>$id
        ));
	...
	public function IjinList($id,$tipe){        
		...
		foreach($result as $value){
			$sts2=$this->db->prepare("SELECT * FROM  `ijinimg` where IjinID='".$value["ID"]."'");
			$sts2->execute();
			//return $sts->fetchAll();
			 $sts2->setFetchMode(PDO::FETCH_ASSOC);
			$data[]=array(
			...
			'StartPeriode'=>$value["StartPeriode"],
			'EndPeriode'=>$value["EndPeriode"],
			'Ket'=>$value["Ket"],
			'lsimg'=>$sts2->fetchAll()
			);
	...
	//echo $sql;
		...
		//$sts->setFetchMode(PDO::FETCH_ASSOC);
		if($sts->rowCount()==0){
			$result=array('CustomerName'=>'','CustomerID'=>"");
			return $result;
		}else{
		$result=$sts->fetch();
		return $result;
	...
	public function UpMateri($data){
        $sts=$this->db->prepare("update `materi` set `ProductID`=:pid,`CustomerID`=:cid,`Tema`=:tema,`tgltayang`=:tgltayang,`pemasang`=:pemasang,`tglfoto`=:tglfoto,`tglupload`=:tglupload where `ID`=:id");
        $sts->execute(array(
            ':id'=>$data['id'],
			':pid'=>$data['pid'],
			':pemasang'=>$data['pemasang'],
	...
	public function updocklist(){    
        ...
        foreach ($mlist as $result){
			...
			if(Code::GetField("customer","CustomerName","CustomerID=".$result['CustomerID'])==""){
					$Cst="";
			}else{
	...
	public function updocklst($id){    
		
        $sts=$this->db->prepare("select * from materi where id='".$id."'");
		//echo "select * from materi where id='".$id."'";
        $sts->execute();
		//$sts->setFetchMode(PDO::FETCH_ASSOC);
		$mlist=$sts->fetchAll();
		return $mlist;
	}
	public function updockDel($id){
        //select Link Image
        $sql = "select * from materiimg where materiID='".$id."'";
        $q = $this->db->prepare($sql);
        $q->execute();
        $q->setFetchMode(PDO::FETCH_BOTH);	
		while($r = $q->fetch()){
        ...
        // remove dari database         
        $sts=$this->db->prepare("Delete from materiimg where materiID='".$id."'");
        $sts->execute();
        $sts=$this->db->prepare("Delete from materi where ID='".$id."'");
        $sts->execute();
    }
...
//echo Code::GetField("customer","Email","CustomerID='".$cst."'");
	...
	public function ListSewa($id,$sbln,$sthn){
		...
		foreach($result as $value){
			...
			$sqlimg="select * from sewaimg where SewaID='".$value["ID"]."'";
			$sts2=$this->db->prepare($sqlimg);
			$sts2->execute();
			$sts2->setFetchMode(PDO::FETCH_ASSOC);
			$data[]=array(
			'ID'=>$value["ID"],
			'NoPerjanjian'=>$value["NoPerjanjian"],
			'Tanggal'=>$value["Tanggal"],
	...
	public function ListKontrak($id,$sbln,$sthn){
		...
		foreach($result as $value){
			...
			$sqlimg="select * from KontrakImg where KontrakID='".$value["KontrakID"]."'";
			$sts2=$this->db->prepare($sqlimg);
			$sts2->execute();
			$sts2->setFetchMode(PDO::FETCH_ASSOC);
			$data[]=array(
			'KontrakID'=>$value["KontrakID"],
	...
	public function ProductViewx($idx){
		...
		$pageLayout = array('800px', '1300px'); //  or array($height, $width) 
		$pdf = new TCPDF('p', 'pt', $pageLayout, true, 'UTF-8', false);
		$pdf->SetCreator("VISTAMEDIA.CO.ID");
		$pdf->SetAuthor('VISTAMEDIA.CO.ID');
...
$html='';
$ulink="";
				foreach($resultx as $prodx){
						foreach($prodx as $prod){
	$lat= preg_replace('/\s+/', '', $prod["Latitude"]);
	$long= preg_replace('/\s+/', '', $prod["Longitude"]);

$html=$html.'
<table width="580" border="0" cellpadding="2" cellspacing="10">
  <tr>
    <td colspan="2" scope="row"><table width="540" border="0">
      <tr>
        <td width="100" scope="row"><img src="'.URL.'/logo.png" width="80" height="50"></td>
        <td width="480" align="left" valign="top"><div align="left">
            <span style="font-size: 11px;font-family: Verdana, Arial, Helvetica, sans-serif;
	font-weight: bold;">PT.VISTAMEDIA</span><br><span style="font-size: 9px;	font-family: Verdana, Arial, Helvetica, sans-serif;">Jl. Pulau Kawe No.40 - Denpsar | Bali <br>Ph. +62 361 5572727 Fax. +62 361 5573737 <br>Email : info@vistamedia.co.id | website : www.vistamedia.co.id</span>
        </div></td>
      </tr>
  ...
  <tr>
	...
	<table width="550" border="1" cellpadding="5" cellspacing="0">
      <tr>
		...
		<table width="540" border="0">
        <tr>
        <td align="left" valign="middle" scope="row" width="100">		
		<div align="left"><span style="color: #FF0000;font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 14px;vertical-align: top;text-align: left;font-weight: bold;">'.strtoupper($prod["ProductCode"]).'</span></div>
		</td> 
		<td align="left" valign="middle" scope="row"  width="75">
		<img src="'.URL.'/assets/img/title-top.jpg" width="70" height="20" align="absmiddle" style="vertical-align: top;text-align: left;font-weight: bold;">
		</td> 
		<td align="left" valign="middle" scope="row" width="365"><span style="	font-size: 10px;
	font-family: Verdana, Arial, Helvetica, sans-serif;
"> &nbsp;<br>'.$prod["Address"].'</span>
</td>
          </tr>
        </table>
</td>
          ...
          <tr>
            <td colspan="2" align="left" valign="middle" scope="row"  style="padding: 5px;vertical-align: top;text-align: left;"><img src="'.URL.$prod['img'].'" height="300" width="560"><br><span style="	font-size: 8px;
	font-family: Verdana, Arial, Helvetica, sans-serif;
">Koordinat : '.$lat.','.$long.'</span></td>
          </tr>
        </table></td>
      </tr>
    </table></td>
  ...
  <tr>
    <td width="300" align="left" valign="top" scope="row"><table border="0">
      <tr>
        <td width="300" scope="row">
		<table>
		<tr><td width="200"><div align="left"><span style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 12px;">Info Area</span></div></td>
        <td width="100"><div align="right"><img src="'.URL.'/assets/img/title-a.jpg" width="80" height="15"></div></td>
      </tr>
	  </table>
	  </td></tr>
      <tr>
	...
	font-size: 10px;padding: 5px;vertical-align: top;text-align: left;">Rute Jalan </td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;"> : </td>
						<td width="280" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;"> '. $prod["RuteJalan"].'</td>
					</tr>
					<tr>
						<td  width="100"   style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;">Akses Menuju </td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;"> : </td>
						<td width="280" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;">'. $prod["akses"].'</td>
					</tr>
					<tr>
						<td width="100"   style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;">Jarak Pandang </td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;"> : </td>
						<td width="280" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;"> '. $prod["JarakPandang"].'</td>
					</tr>
					<tr>
						<td width="100"   style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;">Kecepatan Kendaraan </td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;"> : </td>
						<td width="280" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;"> '. $prod["KecepatanKendaraan"].'</td>
					</tr>
					<tr>
						<td width="100"   style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;">Kawasan</td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;"> : </td>
						<td width="280" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;vertical-align: top;text-align: left;"> '. $prod["Kawasan"].'</td>
					</tr>
</table></td></tr>
</table></td>
        </tr>
    </table></td>
    <td width="240" align="left" valign="top" scope="row"><table border="0">
      <tr>
        <td width="140" scope="row"><div align="left"><span style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 12px;">Info Area</span></div></td>
        <td width="100"><div align="right"><img src="'.URL.'/assets/img/title-a.jpg" width="80" height="15"></div></td>
      </tr>
      <tr>
        <td colspan="2" scope="row"><table  border="1" cellpadding="5" cellspacing="0">
					<tr><td>
					<table>
					<tr>
						<td width="60" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;">Jenis</td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;"> : </td>
						<td width="170" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;"> '.$prod["NamaJenis"].'</td>
					</tr>
					<tr>
						<td width="60" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;">Ukuran</td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;"> : </td>
						<td width="170" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;"> '.$prod["Ukuran"].'</td>
					</tr>
					<tr>
						<td width="60" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;">Jumlah </td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;"> : </td>
						<td width="170" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;"> '.$prod["Jumlah"].'</td>
					</tr>
					<tr>
						<td width="60" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;">Orientasi </td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;"> : </td>
						<td width="170" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;">';
					...
					<tr>
						<td width="60" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;">Penerangan</td>
						<td width="10" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;"> : </td>
						<td width="170" style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;"> ';
		  ...
		  $html=$html.'</td></tr></table></td></tr></table></td>
      </tr>
    </table></td>
  </tr>
  <tr>
    <td colspan="2" scope="row"><table width="560" border="0">
      <tr>
        <td width="450" scope="row"><div align="left"><span style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 12px;">Keterangan Lokasi</span></div></td>
        <td width="100"><div align="right"><img src="'.URL.'/assets/img/title-a.jpg" width="80" height="15"></div></td>
      </tr>
      <tr>
        <td colspan="2" scope="row"><table  border="1" cellpadding="5" cellspacing="0">
					<tr><td style="font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 10px;padding:5px;">'.$prod["Keterangan"].'</td></tr></table></td>
      </tr>
    </table></td>
  </tr>
  <tr>
    ...
    $src = 'https://maps.googleapis.com/maps/api/staticmap?center='.$lat.','.$long.'&zoom=16&scale=1&size=560x350&maptype=roadmap&key='.GMAPKEY.'&markers=size:mid|color:0xff0000|label:|'.$lat.','.$long.'';
	//echo $src;
    $time = time();
    $desFolder = $_SERVER['DOCUMENT_ROOT'].'vistamedia/images/';
    $imageName = 'google-map_'.$time.'.JPG';
    $imagePath = $desFolder.$imageName;
    file_put_contents($imagePath,file_get_contents($src));
	$ulink=$ulink.'|'.$imagePath;
	$html=$html.'<img width="560" src="'.$imagePath.'">
	</td>
  </tr>
  <tr>
    <td colspan="2" scope="row">';
	
    $src2 = 'https://maps.googleapis.com/maps/api/staticmap?center='.$lat.','.$long.'&zoom=16&scale=1&size=560x350&maptype=satellite&key='.GMAPKEY.'&markers=size:mid|color:0xff0000|label:|'.$lat.','.$long.'';
	//echo $src;
    $time2 = time();
    
    $imageName2 = 'google-map_'.$time2.'.JPG';
    $imagePath2 = $desFolder.$imageName2;
    file_put_contents($imagePath2,file_get_contents($src2));
	$ulink=$ulink."|".$imagePath2;
	$html=$html.'<img width="560" src="'.$imagePath2.'"></td>
  </tr>
  <tr>
    <td colspan="2" scope="row" height="100">
	</td>
  </tr>
</table>';
...
$pdf->Output();
$unlinkx=explode("|",$ulinks);
foreach($unlinkx as $delite){
	unlink($delite);
}
//unlink($imagePath);
	...
	public function FreeP($id){
				$now=date('Y-m-d');
				//echo $id;
		if($id==0){
			$sqlx="select product.ID,product.ProductCode,product.Address,product.Kota,product.JenisID,product.Ukuran,product.Jumlah,product.Orientasi,product.Penerangan,product.Side, jenisproduct.NamaJenis,area.NamaArea from (product INNER JOIN jenisproduct ON product.JenisID=jenisproduct.JenisID) inner join area on area.ID=product.AreaID where  product.status=0";
		}else{
				$sqlx="select product.ID,product.ProductCode,product.Address,product.Kota,product.JenisID,product.Ukuran,product.Jumlah,product.Orientasi,product.Penerangan,product.Side, jenisproduct.NamaJenis,area.NamaArea from (product INNER JOIN jenisproduct ON product.JenisID=jenisproduct.JenisID) inner join area on area.ID=product.AreaID where  product.status=0 and area.ID=".$id;
		}
				//echo $sqlx;
        $sts=$this->db->prepare($sqlx);
        $sts->execute();
		$sts->setFetchMode(PDO::FETCH_ASSOC);
		$result=$sts->fetchAll();
		//print_r($result);
		...
		if(count(array_filter($mylist))==0){
			echo "Data Kosong";
		}else{
			$data="";
		foreach ($mylist as $key => $value){
			if(count($value["ID"])<0){}else{			
			$data=$data. '<div id="mexp" class="col-lg-3"><a id="example5" href="#" ><img src="'.URL.$value['img'].'" alt="" width="200px;" height="auto" /></a><br> <input type="checkbox" name="ch'.$xx.'" value="'.$value["ID"].'">'.$value["ProductCode"].'<br>'.$value["Address"].'</div>';
			}
			$xx++;
		}
		$data=$data.'<input type="hidden" class="form-control" id="ci" name="ci" required value="'.$xx.'">';
		print_r($data);
		}
	}
	
	public function PViewx($idx){
		$idm=explode("-",$idx);
		foreach($idm as $id){
